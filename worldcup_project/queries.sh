...
echo "$($PSQL "SELECT name FROM teams LEFT JOIN games ON teams.team_id = games.opponent_id OR teams.team_id = games.winner_id WHERE (year=2014 AND round='Eighth-Final') ORDER BY name;")"
